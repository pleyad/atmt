...
src=fr
tgt=en
data=$base/data/$tgt-$src-vanilla-spm/
spm=$1
...
if ! test -f "$spm"; then
    spm_train --input=$data/preprocessed/train.$src,$data/preprocessed/train.$tgt --user_defined_symbols="&apos;","&quot;","&amp;" --model_prefix=$pwd/$spm --vocab_size=4000 --model_type=bpe
fi
